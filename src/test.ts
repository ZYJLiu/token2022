...
describe("Test Wallets", () => {
  ...
  it("Create Mint with Default Account State (frozen)", async () => {
    const mintKeypair = Keypair.generate();
    const mint = mintKeypair.publicKey;
    ...
    // Create transaction with instructions to create new mint account default account state
    const transaction = new Transaction().add(
      // Invoke system program to create account, transfer owner to token 2022 program
      SystemProgram.createAccount({
        fromPubkey: wallet_1.publicKey,
    ...
    console.log("Create New Mint Account with Default Account State (frozen)");
    console.log(`https://explorer.solana.com/tx/${txSig}?cluster=devnet`);
    console.log(`\n`);

    const txSig2 = await updateDefaultAccountState(
      connection,
      wallet_1, // payer
      mint,
    ...
    console.log("Update Default Account State");
    console.log(`https://explorer.solana.com/tx/${txSig2}?cluster=devnet`);
    console.log(`\n`);
  });

  it("Create Token Account with Immutable Owner", async () => {
    const decimals = 9;
    ...
    const txSig = await sendAndConfirmTransaction(
      connection,
      transaction,
      [wallet_1, accountKeypair],
      undefined
    );

    console.log("Create Token Account with Immutable Owner");
    console.log(`https://explorer.solana.com/tx/${txSig}?cluster=devnet`);
    console.log(`\n`);
  });
});
